        ...
        if parts[0] == "CH" && parts.len() > 5 {
            ...
            if let Some(type_) = match parts[2].as_str() {
                ...
                "VOLENVPER" => {
                    ...
                    Some(Type::Vol{ volume, add, period})
                },
                "DUTYLL" => {
                ...
                &_ => { /* There is a lot of other noise in stdouts so this isn't necessarily an error */ None  }
            } {
...
fn write_msb(m: &mut MachineState, addr: u16, trigger: bool, length_enable: bool, frequency: u8) {
    ...
    m.memory.write_u8(addr, trigger | length_enable | frequency, &mut m.cpu.registers);
}
...
fn write_voladdperiod(m: &mut MachineState, addr: u16, volume: u8, add: bool, period: u8) {
    ...
    m.memory.write_u8(addr, volume | add | period, &mut m.cpu.registers);
}
...
fn write_duty(m: &mut MachineState, addr: u16, duty: u8, load_length: u8) {
    ...
    println!("DUTY: {}", duty);
    m.memory.write_u8(addr, duty | load_length, &mut m.cpu.registers);
}
...
fn main() -> Result<(), Box<dyn Error>> {
  ...
  while instructions.len() > next {
    ...
    if elapsed > instructions[next].at {
        ...
        match todo.type_ {
            Type::Lsb { frequency } => {
                write_lsb(&mut gameboy_state, base_address(todo.channel) + 2, frequency);
            },
            Type::Msb { trigger, length_enable, frequency } => {
                write_msb(&mut gameboy_state, base_address(todo.channel) + 3, trigger, length_enable, frequency);
            },
            Type::Vol { volume, add, period } => {
                write_voladdperiod(&mut gameboy_state, base_address(todo.channel) + 1, volume, add, period);
            },
    ...
    gameboy_state.sound.step(&mut gameboy_state.cpu, &mut gameboy_state.memory, sample_rate, &sound_tx);
  }
